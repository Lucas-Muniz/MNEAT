...
include(joinpath(SRC_PATH, "datasets/Drug-Consumism-Classification/dataset4.jl"))
...
Random.seed!(1234)
net1 = MLPClassifier(dataset4_nfeatures, dataset4_labels, topology)
net2 = deepcopy(net1)

path = joinpath(SRC_PATH, "datasets/Drug-Consumism-Classification")
dt = get_dataset4_setup(path, normalise=true, separate=true)
dt_b = get_dataset4_setup(path, normalise=true, separate=true, balance=true)
...
=#
# Criar uma função para sput up with meaningalvar as redes em um arquivo


#=
path = joinpath(SRC_PATH, "datasets/Drug-Consumism-Classification")
dt = get_dataset4_setup(path, normalise=true, separate=true, balance=true)
...
points_IP = calculate_IP_average_points(dt.n_features, dt.labels, topology, number_nets, data, epochs, step_len, nbins)
generate_plot_IP_average(points_IP, number_nets, epochs, step_len, "IP-dt4-MLP-$(nbins)bins.png")
